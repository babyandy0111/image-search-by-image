...
class MilvusHelper:
    def __init__(self, host=MILVUS_HOST, port=MILVUS_PORT):
        try:
            ...
            print(f"Successfully connect to Milvus with IP:{MILVUS_HOST} and PORT:{MILVUS_PORT}")
        except Exception as e:
            print(f"Failed to connect Milvus __init__: {e}")
            sys.exit(1)
    ...
    def has_collection(self, collection_name):
        ...
        try:
            ...
            print(f"Failed to load data to Milvus: {e}")
            sys.exit(1)
    ...
    def create_collection(self, collection_name):
        ...
        try:
            if not self.has_collection(collection_name):
                ...
                print(f"Create Milvus collection: {collection_name}")
            else:
    ...
    def search_vectors(self, collection_name, top_k):
        # Search vector in milvus collection
        try:
            self.set_collection(collection_name)
            search_params = {"metric_type": METRIC_TYPE, "params": {"nprobe": 16}}
            res = self.collection.search(data=[[0.1, 0.2]], anns_field="embedding", param=search_params, limit=top_k)
            print(f"Successfully search in collection: {res}")
            return res
        except Exception as e:
